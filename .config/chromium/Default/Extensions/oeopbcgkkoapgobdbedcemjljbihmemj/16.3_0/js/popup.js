...
function initFixedArea() {
    ...
    var width = inNotificationWindow ? 18 : 19; // must balance with scrollbar width ie. :-webkit-scrollbar width:xxx 
...
function refresh() {
	...
	if (navigator.platform.toLowerCase().indexOf("xxxmac") != -1 || navigator.platform.toLowerCase().indexOf("linux") != -1 || navigator.platform.toLowerCase().indexOf("unix") != -1) {
		location.reload();
	} else {
	    if ($("#contacts").is(":visible")) {
	 	   ...
	 	   var rotationInterval = rotate($("#refresh"), {forever:true});
		   $(".statusMessageWrapper").show();
...
$(document).ready(function () {
	...
	if (!localStorage.clickedShare && installDate && installDate.diffInDays() <= -3 && installDate.diffInDays() > -10) { // between 3 and 10 days
		...
		$("#eligibleForReducedDonation").show();
	...
	if (Settings.read("showOptionsButton")) {
		$("#options").click(function() {
			...
			sendGA(['_trackEvent', "options", "click"]);
			window.close();
		});
	} else {
		$("#options").hide();
   ...
   if (navigator.platform && (navigator.platform.toLowerCase().indexOf("mac") != -1 || navigator.platform.toLowerCase().indexOf("linux") != -1 || navigator.platform.toLowerCase().indexOf("unix") != -1)) {
	...
	if (Settings.read("voiceNotification")) {
	   if (isMutedByDuration()) {
		   $("#muteVoice").addClass("muted");
	   }
	   
	   $("#muteVoice")
	   		.mouseenter(function() {
	   			muteVoiceDropDownShowTimeout = setTimeout(function() {
	   				if (isMutedByDuration()) {
	   					$("#muteVoiceDropDown").find('div[duration="now"]').hide();
	   					$("#muteVoiceDropDown").find('div[duration="unmute"]').show();
	   				} else {
	   					$("#muteVoiceDropDown").find('div[duration="now"]').show();
	   					$("#muteVoiceDropDown").find('div[duration="unmute"]').hide();
	   				}
				  	$("#muteVoiceDropDown").slideDown();
			  }, 300);
		   })
		   .mouseleave(function() {
			   console.log("speak out");
			   clearTimeout(muteVoiceDropDownShowTimeout);
			   setTimeout(function() {
				   if (!hoveringInMuteVoiceDropDown) {
				   	  $("#muteVoiceDropDown").slideUp();
				   }
			   }, 100);
		   })
		   .click(function() {
			   ...
			   clearTimeout(muteVoiceDropDownShowTimeout);
			   $("#muteVoiceDropDown").hide();
		   })
	   ;
	   
	   $("#muteVoiceDropDown").mouseenter(function() {
		   console.log("dropdown enter")
		   hoveringInMuteVoiceDropDown = true;
	   }).mouseleave(function() {
		   console.log("dropdown leave")
		   hoveringInMuteVoiceDropDown = false;
		   $("#muteVoice").mouseleave();
	   });
	   
	   $("#muteVoiceDropDown div").click(function() {
		   ...
		   if (duration == "now") {
			   ...
			   dateOffset.setMinutes(dateOffset.getMinutes() + parseInt(duration));
