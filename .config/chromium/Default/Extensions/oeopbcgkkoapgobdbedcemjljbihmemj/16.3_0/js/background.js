...
chrome.runtime.onInstalled.addListener(function(details) {
	...
	if (details.reason == "install") {
		...
		if (!localStorage.disabledExtensionUpdateNotifications && (previousVersionObj.major != currentVersionObj.major || previousVersionObj.minor != currentVersionObj.minor)) {
			...
			// legacy: ignore updates notifications for users "from" this version, because we already notified them
			if (details.previousVersion != "16.2") {
				var options = {
						type: "basic",
...
function generateNotificationButton(buttons, buttonsWithValues, value, emails) {
	if (value) {
		...
		if (value == "markAsRead") {
			...
			button = {title:maybeAppendAllMsg(getMessage("delete"), emails), iconUrl:"images/buttons/original/trash.png"};
		} else if (value == "archive") {
			button = {title:maybeAppendAllMsg(getMessage("archiveLink"), emails), iconUrl:"images/buttons/original/archive.png"};
		} else if (value == "spam") {
			button = {title:maybeAppendAllMsg(getMessage("spamLinkTitle"), emails), iconUrl:"images/buttons/original/spam.png"};
		} else if (value == "star") {
			button = {title:maybeAppendAllMsg(getMessage("starLinkTitle"), emails), iconUrl:"images/star-hover4.png"};
		} else if (value == "open") {
			button = {title:getMessage("open"), iconUrl:"images/buttons/original/open.png"};
			...
			button = {title:"Good news long time user! Extras are only 50c click to see/hide this."};
		}
...
function shortcutNotApplicableAtThisTime(title) {
	...
	notif.onclick = function() {
		chrome.tabs.create({ url: "http://jasonsavard.com/faq/Checker_Plus_for_Gmail#Keyboard_shortcuts" });
		this.cancel();
	}
	notif.show();
}
