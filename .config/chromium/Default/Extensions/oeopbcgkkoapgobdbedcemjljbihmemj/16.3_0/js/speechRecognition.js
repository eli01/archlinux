...
function initWarningDialog(message) {
	...
	if ($speechDialog.length) {
		...
		//$speechDialog = $("<div class='speechWarningDialog'><div class='dialogMessage'>" + message + "</div><div class='speechWarningDialogContinue T-I J-J5-Ji aoO T-I-atl L3 T-I-JW' tabindex=99 role='button' style='-webkit-user-select: none'>Resume</div> <div class='speechWarningDialogStop T-I J-J5-Ji aFh T-I-ax7 ar7' act='16' role='button' tabindex=100 style='-webkit-user-select: none;'>Cancel</div></div>");
		// took the markup from the error dialog box when clicking send email that has no recipients
		$speechDialog = $("<div class='speechWarningDialog Kj-JD' tabindex=0 style='opacity: 1' role='alertdialog'><div class='Kj-JD-K7 Kj-JD-K7-GIHV4' ><span class='Kj-JD-K7-K0'>" + message + "</span> <a style='font-size:11px' target='_blank' href='http://jasonsavard.com/faq/Voice_input#Speech_recognition_stopped'>Why?</a></div><!--div class='Kj-JD-Jz'>Please specify at least one recipient.</div--><div class='Kj-JD-Jl'><button data-tooltip='(Enter)' name='ok' class='speechWarningDialogContinue J-at1-auR'>Continue</button> <button data-tooltip='(Esc)' name='cancel' class='speechWarningDialogStop J-at1-auR'>Cancel</button></div></div>");
...
function placeCursorBefore($node) {
	...
	if (sel.rangeCount) {
		...
		range.setStartBefore($node.get(0));
		range.collapse(true);
		sel.removeAllRanges();
		sel.addRange(range);
	}
...
recognition.onresult = function (event) {
	...
	if (event.resultIndex == resultIndexToIgnore) {
		...
		var listeningNodeIsInputTag = $listeningNode.length && $listeningNode.get(0).tagName == "INPUT";
...
function showDropDown($viPhrase, params) {
	...
	viPhraseEnterTimeout = setTimeout(function() {
		...
		if (getSelection().isCollapsed) { //!$viPhrase.hasClass("selected") 
			...
			console.log("rects:", $viPhrase.get(0).getClientRects());
			
			// place the dropdown below selected phrase
			var MARGIN_LEFT_WIDTH = 2;
			var top;
			if ($viPhrase.get(0).getClientRects().length == 1) { // 1 box = not wrapped so place it just at the bottom of this text
				top = $viPhrase.get(0).getClientRects()[0].bottom;
				left = $viPhrase.get(0).getClientRects()[0].left;
			} else { // 2+ means it is wrapped so place it "before" the last rectangle
				//top = $viPhrase.get(0).getClientRects()[$viPhrase.get(0).getClientRects().length-2].bottom;
				top = lastMousePosition.clientY + 7; // + $viPhrase.get(0).getClientRects()[0].height;
			...
			$dropdown.offset({top:offsetTop, left:$viPhrase.get(0).getClientRects()[0].left});
			*/
		}
...
function shouldCapitalize(range, text) {
	...
	if (range.startContainer.hasAttribute && range.startContainer.hasAttribute("contenteditable")) {
		capitalize = true;
		...
		if (range.startContainer.previousSibling == null) {					
		...
		} else if (range.startContainer.previousSibling.nodeType == Node.TEXT_NODE) {
			if (range.startContainer.previousSibling.nodeValue.sentenceEnd()) {
				capitalize = true;
			...
			if (range.startContainer.previousSibling.innerText == "" || range.startContainer.previousSibling.innerHTML.sentenceEnd()) {
				capitalize = true;
				console.log("capitalize8: " + text);
			} else {
				console.log("html: " + range.startContainer.previousSibling.innerHTML);
				console.log("text: " + range.startContainer.previousSibling.innerText);
				console.log("range:", range);
				console.log("nocap4");
			}
		}
		...
		if (sel.rangeCount) {
			...
			if (params.interimFlag) {
				...
				if ($interimNode) {
					...
					if ($.trim($interimNode.get(0).innerHTML) == "") {
						...
						if (sel.focusNode.nodeType == Node.TEXT_NODE && sel.focusOffset == sel.focusNode.length) {
							...
							} else if ($parent.get(0).tagName == "DIV") { // this happens without starting mic and we enter text on several lines and contenteditable area wraps lines with DIVs
								console.log("in div")
								range.insertNode( $interimNode.get(0) );
							} else {
