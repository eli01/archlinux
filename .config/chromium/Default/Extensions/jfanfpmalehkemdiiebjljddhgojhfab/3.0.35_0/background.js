	...
	var loginSuccess = function (responseJSON) {
		try {
			if (responseJSON.code != '200') {
				port && port.postMessage(responseJSON.message);
				return;
			}
...
function wizPostDocument(docInfo) {
	...
	var updateData = 'api_version=3&client_type=webclip_chrome&token=' + getReplaceStr(Wiz_Context.token) + '&kb_guid=' + getReplaceStr(Wiz_Context.kbGuid)
						...
						+ '&document_title=' + title + '&document_url=' + getReplaceStr(docInfo.url);
	//发送给当前tab消息，显示剪辑结果					
...
function isLogin() {
	if (Wiz_Context.token === null || Wiz_Context.token === "") {
		return false;
	} 
	return true;
}
