...
function LoginControl() {
	...
	function checkEmail() {
		...
		if (!valid) {
			$('#div_error_validator').html(chrome.i18n.getMessage('userid_error')).show(100);
		}
		return valid;
	...
	function checkPassword() {
		...
		if (password.trim().length < 1) {
			$('#div_error_validator').html(chrome.i18n.getMessage('password_error')).show(100);
			return false;
		}
		return true;
	...
	function initCreateAccountLink() {
		$('#create_acount').html(chrome.i18n.getMessage('create_account_link')).bind('click', function(evt) {
			window.open('http://note.wiz.cn/register');
		});
	}
