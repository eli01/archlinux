...
GlobalUtils.setStyleSrc2Target = function(s,t) {
	...
	var css = window.getComputedStyle(s, null), v;
	for (var key in css) {
		if (!key.match(defaultStyle)) {
			continue;
		}
		v = css.getPropertyValue(key);
		if (css.hasOwnProperty(key) && !!v) {
			t.style[key] = v;
		}
	}
};
GlobalUtils.setAllStyleSrc2Target = function(src,target) {
	...
	var childNodes = target.children, i,j;
	for (i=0, j=childNodes.length; i<j; i++) {
		GlobalUtils.setAllStyleSrc2Target(childNodes[i], childNodes[i]);
	}
};
