...
var ClientClipper = function () {
	...
	function wiz_base64Encode(str) {
		...
		// //临时解决方案，处理保存整页时会把 遮罩 canvas 保存进去的 bug
		// var canvasReg = new RegExp('<canvas[^<>]*9999999999999990;[^<>]*>', 'i');
		// str = str.replace(canvasReg, '');
	...
	function wiz_getFrameNodes(win) {
		...
		if (doc == null)
			return null;
		//
		var result = []; frameNodes = doc.getElementsByTagName("iframe");
	...
	function wiz_collectAllFrames(win) {//
		...
		if ( typeof (win) == "object") {
			...
			var htmlStr = win.document.documentElement.innerHTML;
			...
			var source_html = [];
			source_html.push(docType + '<html>', htmlStr, '</html>');
			var source_html = wiz_base64Encode(source_html.join(''));
	...
	function wiz_getSelected(win, isNative) {
		...
		if ( typeof (win) == "object") {
			...
			if (winsel == null || winsel == "") {
				...
				var source_html = (_winsel.outerHTML) ? (_winsel.outerHTML) : _winsel;
				if (source_html == null)
					source_html = "";
				if (isNative === true) {
	...
	function getSelectedHTML() {
		...
		if (selection.rangeCount > 0) {
			...
			var _document = range.commonAncestorContainer.ownerDocument;
